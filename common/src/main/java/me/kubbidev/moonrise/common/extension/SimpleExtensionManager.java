...
import me.kubbidev.api.extension.ExtensionManager;
...
public class SimpleExtensionManager implements ExtensionManager, AutoCloseable {
    ...
    public SimpleExtensionManager(MoonRisePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public @NotNull Extension loadExtension(Path path) throws IOException {
        ...
        try (JarFile jar = new JarFile(path.toFile())) {
            ...
            var extensionJarEntry = jar.getJarEntry("extension.json");
        ...
        try {
            Constructor<? extends Extension> constructor = extensionClass.getConstructor(MoonRise.class);
    ...
    private record LoadedExtension(Extension instance, Path path) {
    }

    private static boolean isJarInJar() {
        String thisClassLoaderName = SimpleExtensionManager.class.getClassLoader().getClass().getName();
        return thisClassLoaderName.equals("me.kubbidev.moonrise.common.loader.JarInJarClassLoader");
    }
