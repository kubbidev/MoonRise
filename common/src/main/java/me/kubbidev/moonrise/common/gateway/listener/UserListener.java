...
public class UserListener extends ListenerAdapter {
    ...
    public UserListener(MoonRisePlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private <T> void handleUserUpdate(User user, T oldValue, T newValue, Consumer<ApiUser> action, String errorMessage) {
        ...
        this.plugin.getGatewayClient().modifyUser(user, action)
                .exceptionally(t -> {
    ...
    private void updateUserLastSeen(User user) {
        this.plugin.getGatewayClient()
                .modifyUser(user, u -> u.setLastSeen(Instant.now()))
                .exceptionally(t -> {
                    this.plugin.getLogger().warn("An error occurred while updating member last seen", t);
                    return null;
                });
    }
