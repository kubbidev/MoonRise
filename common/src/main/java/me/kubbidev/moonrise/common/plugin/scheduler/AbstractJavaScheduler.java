...
public abstract class AbstractJavaScheduler implements SchedulerAdapter {
    ...
    public AbstractJavaScheduler(MoonRiseBootstrap bootstrap) {
        ...
        this.scheduler = new ScheduledThreadPoolExecutor(1, r -> {
            ...
            thread.setName("moonrise-scheduler");
            return thread;
        });
    ...
    public void shutdownScheduler() {
        ...
        try {
            if (!this.scheduler.awaitTermination(1, TimeUnit.MINUTES)) {
                this.bootstrap.getPluginLogger().severe("Timed out waiting for the MoonRise scheduler to terminate");
                reportRunningTasks(thread -> thread.getName().equals("moonrise-scheduler"));
            }
    ...
    private static final class WorkerThreadFactory implements ForkJoinPool.ForkJoinWorkerThreadFactory {
        ...
        public ForkJoinWorkerThread newThread(ForkJoinPool pool) {
            ...
            thread.setName("moonrise-worker-" + COUNT.getAndIncrement());
            return thread;
        }
    }
