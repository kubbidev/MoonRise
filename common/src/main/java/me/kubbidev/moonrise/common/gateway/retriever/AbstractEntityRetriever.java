...
public abstract class AbstractEntityRetriever implements EntityRetriever {
    ...
    @Override
    public CompletableFuture<ApiUser> modifyUser(User user, Consumer<ApiUser> action) {
        return this.storage.loadUser(user.getIdLong())
                .thenApplyAsync(data -> {
    ...
    @Override
    public CompletableFuture<ApiGuild> modifyGuild(Guild guild, Consumer<ApiGuild> action) {
        return this.storage.loadGuild(guild.getIdLong())
                .thenApplyAsync(data -> {
    ...
    @Override
    public CompletableFuture<ApiMember> modifyMember(Member member, Consumer<ApiMember> action) {
        long guildId = member.getGuild().getIdLong();
        return this.storage.loadMember(guildId, member.getIdLong())
                .thenApplyAsync(data -> {
